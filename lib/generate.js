...
function generatePromptFiles({ maxLength, outputBase, format, includeFolders, templatePath }) {
  ...
  for (const filePath of walk(projectRoot, ig, includeFolders)) {
    ...
    try {
      ...
      console.warn(`❌ Error leyendo archivo ${filePath}: ${err.message}`);
      continue;
    }

    if (!content.trim()) {
      console.log(`⏭️  Ignorado (vacío): ${filePath}`);
      continue;
    }
    ...
    if (entrySize > maxLength) {
      console.warn(`⚠️ Archivo demasiado grande para incluir (${filePath}, ${entrySize} bytes). Saltado.`);
      continue;
    }
