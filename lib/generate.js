...
// ✅ Importante: relPath ahora es relativo a sourcePath, no process.cwd()
function* walk(dir, ig, includeFolders, root) {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      // ✅ Asegurarse de que relPath coincide con includeFolders correctamente
      if (
        includeFolders.length === 0 ||
        includeFolders.some(f => relPath === f || relPath.startsWith(f + path.sep))
      ) {
...
function cleanOldPromptFiles(outputBase, outputDir) {
  const oldFiles = glob.sync(`${outputBase}*.{txt,md,json}`, { cwd: outputDir });
...
function generatePromptFiles({
  ...
  if (!stdout && !fs.existsSync(outputDir)) {
  ...
  if (!stdout) {
    cleanOldPromptFiles(outputBase, outputDir);
  }
  ...
  for (const { relPath, buffer: fileBuffer } of walk(sourcePath, ig, includeFolders, sourcePath)) {
    ...
    if (estimatedSize > maxLength && hasContent) {
      ...
      if (stdout) {
        ...
        const outputPath = path.join(outputDir, `${outputBase}${fileIndex}.${format}`);
  ...
  if (!stdout) {
    console.log(`✅ Archivos generados en formato ${format} en ${outputDir}`);
  }
}
